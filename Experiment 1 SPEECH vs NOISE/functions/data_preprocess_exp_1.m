...
path = [pwd,'\fNIRS data exp1\unprocessed data'];
...
for index=1:length(files)
    ...
    %set s_b, s_speech, s_noise at peaks to 1
    %first 10 is breath holding, last 10 is task
    s_b=zeros(length(s),1);
    s_speech=zeros(length(s),1);
    s_noise=zeros(length(s),1);
       
    peak_b = peak_index_aux1(1:10);
    peak_ITD = peak_index_aux1(end-6+1:end); 
    peak_noITD = peak_index_aux2(end-6+1:end);
 
    s_b(peak_b)=1;
    s_speech(peak_ITD)=1;
    s_noise(peak_noITD)=1;
    ...
    s_noise=s_noise(1:end);
    noise_first_index = min(find(s_noise==1));
    noise_last_index = max(find(s_noise==1));
    
    first_task_index = min([speech_first_index, noise_first_index]);
    last_task_index = max([speech_last_index, noise_last_index]);
    ...
    s_noise_task = s_noise(start_index:end_index);
    ...
    save(subject_data_name,'dc_task','s_speech_task','s_noise_task','dc','s_b','aux','t','first_task_index','last_task_index', 'dod_raw');
