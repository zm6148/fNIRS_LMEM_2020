function data_analysis_LMER_exp1_build_data_for_R(Hb_type)%% load data folder
% data analysis using GLM model
%clear;
%close all;

%% load processed fnirs data
path = [pwd, '\fNIRS data exp1\processed data'];
filepattern=fullfile(path,'/*.mat');
files=dir(filepattern);

%% load behavior data
load([pwd, '\behavioral data exp1\behavioral_data.mat']);
...
for index=1:length(files)
    ...
    for jj = 1 : 4 % 4 regions of interest
        ...
        noise_d_prime = noise_d(index);
        [blk_num, condition, behavior] = block_number_R(s_speech_task, s_noise_task, speech_d_prime, noise_d_prime);
        ...
        fnirs_data_pieced = piece_blk(dc_task(:, number, channels), s_speech_task + s_noise_task)/ max_b;
        
        % piece together RC channel fnirs data
        RC_fnirs_data_pieced = piece_blk(dc_task(:, number, RC), s_speech_task + s_noise_task)/ max_b;
        ...
        % first derivative
        % HRF function for SPEECH and NOISE
        time_window  = 0 : 1/50 : 45;
        [target_hrf_speech, target_hrf_noise, time] = target_HRF_cat_R(condition_no_gap, blk_num_no_gap, Hb_type, time_window);
        first_deriv_speech = [0; diff(target_hrf_speech)];
        first_deriv_noise = [0; diff(target_hrf_noise)];
        second_deriv_speech = [0; diff(first_deriv_speech)];
        second_deriv_noise = [0; diff(first_deriv_noise)];
        ...
        dummy = cat(2, sid, hemsphere, location, roi_code, task_type, ...
            ...
            target_hrf_speech, target_hrf_noise, ...
            first_deriv_speech, first_deriv_noise, ...
            second_deriv_speech, second_deriv_noise, ...
...
% save as cvs
% go up one directory
csvwrite([pwd, '\LMEM R data exp1\R_data_speech_noise_', Hb_type, '.csv'], data_for_R);
end